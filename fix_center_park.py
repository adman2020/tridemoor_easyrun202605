...
print(f'中心公园: {len(gcj)}pts, {total:.2f}km, center=({clat:.4f},{clng:.4f})')
...
for rid, rname in [
    ...
    print(f'-- {rname}')
    print(f"DELETE FROM route_points WHERE route_id = '{rid}';")
    print(f"INSERT INTO route_points (route_id, point_index, latitude, longitude, altitude) VALUES")
    for j, (lat,lng) in enumerate(gcj):
        print(f"  ('{rid}', {j}, {lat:.7f}, {lng:.7f}, 0){',' if j<len(gcj)-1 else ';'}")
    print(f"UPDATE routes SET distance={total:.2f}, start_lat={gcj[0][0]:.7f}, start_lng={gcj[0][1]:.7f}, center_lat={clat:.7f}, center_lng={clng:.7f}, elevation_gain=ROUND({total}*5), avg_pace=420, tags='[\"公园\",\"环线\",\"平路\"]' WHERE id='{rid}';")
    print()
